MODULE sbcisf
   ...
   REAL(wp), PUBLIC ::   rn_vtide                    !: RMS velocity associated with tides for exchange mixing
   ...
   REAL(wp), PUBLIC, SAVE ::   rhoisf = 918.0_wp      ! phycst ?
...
CONTAINS
  ...
  SUBROUTINE sbc_isf(kt)
    ...
    !---temporary fix
    REAL(wp), DIMENSION(:,:), POINTER       ::   ztfrz
 
      !
      !!---------------------------------------------------------------------
      NAMELIST/namsbc_isf/ nn_isfblk, rn_hisf_tbl, ln_divisf, ln_conserve, rn_gammat0, rn_gammas0, nn_gammablk, &
                         & sn_fwfisf, sn_qisf, sn_rnfisf, sn_depmax_isf, sn_depmin_isf, sn_Leff_isf, rn_vtide 
...
902      IF( ios /= 0 ) CALL ctl_nam ( ios , 'namsbc_isf in configuration namelist', lwp )
         ...
         IF ( lwp ) WRITE(numout,*) '        rn_vtide    = ', rn_vtide 
...
!         risf_tsc(:,:,jp_tem) = qisf(:,:) * r1_rau0_rcp - rdivisf * fwfisf(:,:) * zt_frz * r1_rau0 !
         risf_tsc(:,:,jp_tem) = qisf(:,:) * r1_rau0_rcp - rdivisf * fwfisf(:,:) * ztfrz(:,:) * r1_rau0 !
  ...
  END SUBROUTINE sbc_isf_bg03
   ...
   SUBROUTINE sbc_isf_cav( kt, ptfrz )
      !!---------------------------------------------------------------------
      !!                     ***  ROUTINE sbc_isf_cav  ***
      !!
      ...
      !!---------------------------------------------------------------------
      INTEGER, INTENT(in)          ::   kt         ! ocean time step
      REAL(wp), DIMENSION(:,:) , INTENT(out):: ptfrz  ! freezing temperature at the interface
      !
      LOGICAL :: ln_isomip = .true.
      REAL(wp), DIMENSION(:,:), POINTER       ::   zfrz,zpress,zti,zsfrz_A
      ...
      IF (nn_eos == 2) THEN !ISOMIP+ EOS, tfreez calc
        zlamb1=-0.0573
        zlamb2=0.0832
        !zlamb3=-7.53e-04
        zlamb3=-7.53e-08* grav * rau0
        !COEFFICIENTS FOR ISOMIP LINEARISATION
        !UNITS ON ZLAMB3 - XYLAR HAS E-08 C/Pa, ORIG WAS E-04
        !FACTOR OF E04 in decibar to pascal, 1dbar ~ 1m
        !WE DO ZLAMB3*risf_dep
      END IF

      IF( nn_timing == 1 )  CALL timing_start('sbc_isf_cav')
      !
      CALL wrk_alloc( jpi,jpj, zfrz,zpress,zti, zsfrz_A, zgammat2d, zgammas2d )
...
! Calculate in-situ temperature (ref to surface)
      ...
      IF (nn_eos == 2) THEN !ISOMIP+ EOS, tfreez calc
! potential temperature is used to compute the melting
        zti(:,:)=ttbl
      END IF
! Calculate freezing temperature
!      CALL eos_fzp( sss_m(:,:), zfrz(:,:), zpress )
      CALL eos_fzp( sss_m(:,:), zfrz(:,:), risfdep(:,:) ) ! not actually used by ISOMIP+
...
! PM suggests commenting this line out? Was out at r5620, in at r5952?
!!RSS OUT                      zfwflx = ( zgammas*rau0 - zcfac*zfwflx ) * (zsfrz - stbl(ji,jj)) / stbl(ji,jj)
!!
! test convergence and compute gammat
                     IF (( zhtflx - zhtflx_b) .LE. 0.01 ) lit = .FALSE.
...
! If non conservative we have zcfac=0.0 so what follows is then zfwflx*sss_m/zsfrz
      ...
      CALL iom_put('thermald',zti-zfrz)
      CALL iom_put('halined',zsfrz_A-stbl)
      CALL iom_put('ustar',zgammat2d/rn_gammat0)
      !
   ...
   !      zvt = 0.5 * ( vtbl(ji  ,jj-1) + vtbl(ji,jj) )
      ...
      !! compute ustar
         zustar = SQRT( rn_tfri2 * (zut * zut + zvt * zvt + rn_vtide*rn_vtide) )
