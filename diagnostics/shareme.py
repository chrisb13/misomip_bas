...
class Grid(object):
    ...
    titloc (optional): location for subplot (within each subplot) for the title box
    ...
    * Many of the globalcbar options depend on a colour or 'True' being passed to globalcbar.
    ...
    """
    def __init__(self, pdict,pdims,sharex=False,sharey=False,clevels=0,dimlabels={},sepcbar=False,globalclimits=None,globalcbar='False',globalcbarmiddle=False,cbars={},zoom=None,outputpath='',pmask='',titloc=1,figtit=None):
    ...
    def mkplot(self):
        ...
        if self.figtit is not None:
            fig.suptitle(self.figtit)
        ...
        for rownum in range(self.pdims[0]):
            for colnum in range(self.pdims[1]):
                if pnum<=subplotnum:
                        ...
                        if self.clevels!=0:
                            if self.globalcbar=='True':
                                cs1=ax.contourf(field,levels=np.linspace(fgmin,fgmax,self.clevels))
                                    ...
                                    cs1=ax.contourf(field,levels=np.linspace(fgmin,fgmax,self.clevels),cmap=colourmap,extend='both')
                        else:
                            if self.globalcbar=='True':
                    ...
                    # ax.set_title(name)
                    inset_title_box(ax,name,location=self.titloc,bwidth="60%")
